...
router.delete('/user/:userId', (req, res) => {
  ...
  res.set('Content-Type', 'application/xml');
  res.send(
    `<response><message>User data for user ${userId} deleted successfully</message></response>`
  );
});
router.options('/', (req, res) => {
  res.set('Content-Type', 'application/xml');
  res.send(
    '<response><message>Allowed methods: GET, POST, DELETE, PATCH, OPTIONS</message></response>'
  );
});
